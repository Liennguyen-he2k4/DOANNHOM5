...
def attendance_mark_view(request, class_id):
    ...
    # Kiểm tra xem giảng viên có quyền điểm danh lớp này không
...
def enter_score_view(request, class_id):
    ...
    # Tạo danh sách học viên kèm điểm
    student_scores = [
        {
            'student': student,
        ...
        for student in students:
            ...
            if score_key in request.POST:
                try:
                    ...
                    messages.error(request, f"Điểm của {student.username} không hợp lệ.")
                    continue
